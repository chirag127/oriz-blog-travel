...
/** Map a raw category string → one of the five travel desks. */
export function verticalFor(category: string): Vertical {
  const c = category.toLowerCase()
  if (/(safety|solo|scam|health|insur|women|risk|emergency)/.test(c)) return 'health'
  if (/(budget|money|cost|cheap|fund|save|visa|finance|nomad)/.test(c)) return 'finance'
  if (/(route|itiner|trek|guide|transit|trail|city|region|destination|how-to|plan)/.test(c))
    return 'dev'
  if (/(story|culture|food|people|journal|essay|photo|lifestyle|travel)/.test(c)) return 'culture'
  return 'personal'
}
